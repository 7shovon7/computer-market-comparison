...
for item in search_items:
    item_link = item.find(attrs={'class': 'product-content'}).find('a')['href']
...
for link in items:
    ...
    try:
        ...
        for info in info_section:
            try:
                row = info.find('div', attrs={'class': 'row'}).find_all('div')
...
file_name = f"{search_input.replace(' ', '_')}"
file_ext = 'csv'
output_file = generate_file_name(file_name, file_ext)
